...
contract CLSwapRouterTest is TokenFixture, Test {
    ...
    function setUp() public {
        ...
        vault = IVault(address(poolManager));
        ...
        positionManager = new CLPoolManagerRouter(IVault(address(poolManager)), poolManager);
        ...
        router = new MockV4Router(IVault(address(poolManager)), poolManager);
    ...
    function testExactInputSingle_amountOutLessThanExpected() external {
        vm.expectRevert(abi.encodeWithSelector(IV4Router.TooLittleReceived.selector, 2 ether, 996990060009101709));
    ...
    function testExactInput_amountOutLessThanExpected() external {
        vm.expectRevert(abi.encodeWithSelector(IV4Router.TooLittleReceived.selector, 2 ether, 993989209585378125));
        PathKey[] memory path = new PathKey[](2);
        path[0] = PathKey({
            intermediateCurrency: currency1,
            fee: uint24(3000),
    ...
    function testExactOutputSingle_amountOutLessThanExpected() external {
        vm.expectRevert(
            abi.encodeWithSelector(IV4Router.TooMuchRequested.selector, 0.01 ether, 10030190572718166)
        );
    ...
    function testExactOutput_amountInMoreThanExpected() external {
        vm.expectRevert(
            abi.encodeWithSelector(IV4Router.TooMuchRequested.selector, 0.01 ether, 10060472596238902)
        );
