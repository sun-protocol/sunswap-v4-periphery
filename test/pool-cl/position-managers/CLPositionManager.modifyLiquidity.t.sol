...
contract CLPositionManagerModifyLiquiditiesTest is Test, PosmTestSetup, LiquidityFuzzers {
    ...
    function setUp() public {
        ...
        nativeKey =
            PoolKey(CurrencyLibrary.NATIVE, currency1, IHooks(address(0)), 3000, bytes32(uint256(60 << 16)));
        manager.initialize(nativeKey, SQRT_RATIO_1_1);

        wethKey = PoolKey(
            Currency.wrap(address(_WETH9)), currency1, IHooks(address(0)), 3000, bytes32(uint256(60 << 16))
        );
