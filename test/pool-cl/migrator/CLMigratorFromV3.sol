...
import {Pausable} from "v4-core/src/base/Pausable.sol";
...
abstract contract CLMigratorFromV3 is OldVersionHelper, PosmTestSetup, Permit2ApproveHelper {
    ...
    //     ICLMigrator.V4CLPoolParams memory infiMintParams = ICLMigrator.V4CLPoolParams({
    //         poolKey: poolKey,
    //         tickLower: -100,
    //         tickUpper: 100,
    //         liquidityMin: 0,
    ...
    function testCLMigrateFromV3IncludingInit() public {
        ...
        assertApproxEqAbs(address(poolManager).balance, 10 ether, 0.000001 ether);
    ...
    function testCLMigrateFromV3WithoutNativeToken() public {
        ...
        assertApproxEqAbs(token0.balanceOf(address(poolManager)), 10 ether, 0.000001 ether);
        assertApproxEqAbs(token1.balanceOf(address(poolManager)), 10 ether, 0.000001 ether);
    }
    ...
    function testCLMigrateFromV3AddExtraAmount() public {
        ...
        assertApproxEqAbs(address(poolManager).balance, 30 ether, 0.000001 ether);
        assertApproxEqAbs(token0.balanceOf(address(poolManager)), 30 ether, 0.000001 ether);
    }
    ...
    function testFuzz_CLMigrateFromV3AddExtraAmountThroughWETH(uint256 extraAmount) public {
        ...
        assertApproxEqAbs(address(poolManager).balance, 10 ether + extraAmount, 0.000001 ether);
        assertApproxEqAbs(token0.balanceOf(address(poolManager)), 10 ether + extraAmount, 0.000001 ether);
    }
        ...
        public
    {
        ...
        uint256 vaultCurrency0BalanceBefore = poolKey.currency0.balanceOf(address(poolManager));
        uint256 vaultCurrency1BalanceBefore = poolKey.currency1.balanceOf(address(poolManager));
        vm.recordLogs();
        // 4. migrate from v3 to v4, not sending ETH denotes pay by WETH
        migrator.migrateFromV3(v3PoolParams, infiMintParams, extraAmount0, extraAmount1);

        uint256 vaultCurrency0BalanceAfter = poolKey.currency0.balanceOf(address(poolManager));
        uint256 vaultCurrency1BalanceAfter = poolKey.currency1.balanceOf(address(poolManager));
    ...
    function testCLMigrateFromV3Refund() public {
        ...
        assertApproxEqAbs(address(poolManager).balance, 5 ether, 0.1 ether);
        assertApproxEqAbs(token0.balanceOf(address(poolManager)), 5 ether, 0.1 ether);
    }
    ...
    function testCLMigrateFromV3RefundNonNativeToken() public {
        ...
        assertApproxEqAbs(token0.balanceOf(address(poolManager)), 5 ether, 0.1 ether);
        assertApproxEqAbs(token1.balanceOf(address(poolManager)), 5 ether, 0.1 ether);
    }
    ...
    function testCLMigrateFromV3ThroughOffchainSignPayWithETH() public {
        ...
        assertApproxEqAbs(address(poolManager).balance, 20 ether, 0.000001 ether);
        assertApproxEqAbs(token0.balanceOf(address(poolManager)), 20 ether, 0.000001 ether);
    }
