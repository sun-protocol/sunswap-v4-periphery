...
abstract contract CLMigratorFromV2 is OldVersionHelper, PosmTestSetup, Permit2ApproveHelper, Permit2SignatureHelpers {
    ...
    function testCLMigrateFromV2Refund() public {
        ...
        assertApproxEqAbs(address(poolManager).balance, 5 ether, 0.000001 ether);
        assertApproxEqAbs(token0.balanceOf(address(poolManager)), 5 ether, 0.000001 ether);
    }
    ...
    function testCLMigrateFromV2RefundNonNativeToken() public {
        ...
        assertApproxEqAbs(token0.balanceOf(address(poolManager)), 5 ether, 0.000001 ether);
        assertApproxEqAbs(token1.balanceOf(address(poolManager)), 5 ether, 0.000001 ether);
    }
