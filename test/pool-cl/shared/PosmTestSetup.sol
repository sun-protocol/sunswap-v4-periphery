...
contract PosmTestSetup is Test, Deployers, DeployPermit2, CLLiquidityOperations {
    ...
    function deployAndApproveRouter(PoolManager poolManager) public {
    ...
    function deployAndApprovePosm(PoolManager poolManager) public {
        deployPosm(poolManager);
        approvePosm();
    }

    function deployPosm(PoolManager poolManager) internal {
        // We use deployPermit2() to prevent having to use via-ir in this repository.
        permit2 = IAllowanceTransfer(deployPermit2());
        positionDescriptor = new CLPositionDescriptorOffChain("https://sun.io/v4/positions/");
        lpm = new CLPositionManager(poolManager, poolManager, permit2, 100_000, positionDescriptor, IWETH9(_WETH9));
    }
